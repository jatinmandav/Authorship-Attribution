...
class ResLSTM:
    ...
    def residual_block(self, x, block_num, hidden_1, hidden_2):
        ...
        with tf.variable_scope('fw_lstm{}-{}'.format(block_num, 1), reuse=True):
            forward_lstm1 = rnn.BasicLSTMCell(hidden_1, name='fw_lstm{}-{}'.format(block_num, 1))
        with tf.variable_scope('bw_lstm{}-{}'.format(block_num, 2), reuse=True):
            backward_lstm1 = rnn.BasicLSTMCell(hidden_1, name='bw_lstm{}-{}'.format(block_num, 2))
        ...
        with tf.variable_scope('fw_lstm{}-{}'.format(block_num+1, 1), reuse=True):
            forward_lstm2 = rnn.BasicLSTMCell(hidden_2, name='fw_lstm{}-{}'.format(block_num+1, 1))
        with tf.variable_scope('bw_lstm{}-{}'.format(block_num+1, 2), reuse=True):
            backward_lstm2 = rnn.BasicLSTMCell(hidden_2, name='bw_lstm{}-{}'.format(block_num+1, 2))
    ...
    def model(self, x):
        ...
        res3, res4, res_link2 = self.residual_block(res_link1, 2, 32, 64)
